...
<ZJInterfaceDelegate,UITextFieldDelegate>
{
    //    MBProgressHUD *HUD;
}
...
/** 忘记密码接口 */
@property (nonatomic,strong) ZJInterface *interfaceForgetPwd;
/** 手机号输入框 */
...
- (void) requestForgetPwd
{
    ...
    self.interfaceForgetPwd = [[ZJInterface alloc] initWithDelegate:self];
    
    [self.interfaceForgetPwd interfaceWithType:INTERFACE_TYPE_FORGETPWD param:param];
}
...
- (void)interface:(ZJInterface *)interface result:(NSDictionary *)result error:(NSError *)error
{
    ...
    }else if (interface == self.interfaceForgetPwd)
    {
        if ([result[@"code"] isEqual:@(0)]) {
